...
// app is created before component registrations -- see var app = Vue.createApp({...}) below
// globalProperties, $filters, plugins, and components are all set on app after creation.
// The declaration is hoisted here so references in moqui.* helper functions work.
var app;
...
moqui.handleFetchError = function(err) {
    if (err && err.name === 'AbortError') { return; }
    // fetch throws new Error(r.status) for non-ok responses; message is the status code as a string
    ...
    if (moqui.webrootVue && moqui.webrootVue.$q) {
        moqui.webrootVue.$q.notify(Object.assign({}, moqui.notifyOptsError, { message: msg }));
    }
    if (moqui.webrootVue && moqui.webrootVue.addNotify) { moqui.webrootVue.addNotify(msg, 'negative'); }
};
...
moqui.loadComponent = function(urlInfo, callback, divId) {
    ...
    */
    ...
    if (url.slice(-vueExt.length) === vueExt) {
        ...
        var fetchMethod = (bodyParameters && Object.keys(bodyParameters).length > 0) ? 'POST' : 'GET';
        var fetchOptions = { method: fetchMethod, signal: controller.signal };
        if (fetchMethod === 'POST') { fetchOptions.body = new URLSearchParams(bodyParameters); }
        fetch(url, fetchOptions).then(function(r) {
            ...
            return r.text().then(function(resp) {
                ...
                if (moqui.isString(resp) && resp.length > 0) {
                    // Vue 3: defineAsyncComponent required for httpVueLoader.parse() factory function
                    // when used outside the httpVueLoader.install() beforeCreate mixin path.
                    // markRaw prevents the component object from being made reactive when stored
                    // in reactive data (activeComponent, navPlugins, accountPlugins).
                    var vueCompObj = Vue.markRaw(Vue.defineAsyncComponent(httpVueLoader.parse(resp, url.substr(0, url.lastIndexOf('/')+1))));
                    if (isServerStatic) { moqui.componentCache.put(path, vueCompObj); }
                    callback(vueCompObj);
                } else { callback(moqui.NotFound); }
            });
        }).catch(function(err) {
            if (err && err.name === 'AbortError') { console.warn('loadComponent aborted: ' + url); moqui.webrootVue.loading = 0; return; }
            moqui.webrootVue.loading = 0;
            moqui.handleFetchError(err);
        });
    ...
    var ajaxSettings = { type:"GET", url:url, error:moqui.handleLoadError, success: function(resp, status, jqXHR) {
        ...
        if (moqui.isString(resp) && resp.length > 0) {
            if (isJsPath || resp.slice(0,7) === 'define(') {
                ...
                var jsCompObj = Vue.markRaw(eval(resp));
...
// Vue 3: markRaw prevents component definition objects from being made reactive
// when stored in reactive data (activeComponent, navPlugins, accountPlugins).
moqui.NotFound = Vue.markRaw({ template: '<div id="current-page-root"><h4>Screen not found at {{this.$root.currentPath}}</h4></div>' });
moqui.EmptyComponent = Vue.markRaw({ template: '<div id="current-page-root"><div class="spinner"><div>&nbsp;</div></div></div>' });


// Vue 3 clears #apps-root innerHTML before running any lifecycle hooks (created, data).
// Read all conf values from the DOM now, while it is still intact.
var moquiConf = {
    moquiSessionToken: $("#confMoquiSessionToken").val() || '',
    appHost:           $("#confAppHost").val()           || '',
    appRootPath:       $("#confAppRootPath").val()       || '',
    basePath:          $("#confBasePath").val()          || '',
    linkBasePath:      $("#confLinkBasePath").val()      || '',
    userId:            $("#confUserId").val()            || '',
    username:          $("#confUsername").val()          || '',
    locale:            $("#confLocale").val()            || 'en',
    darkMode:          $("#confDarkMode").val()          || 'false',
    leftOpen:          $("#confLeftOpen").val()          === 'true',
    navPluginUrls:     [],
    accountPluginUrls: []
};
$('.confNavPluginUrl').each(function(idx, el) { moquiConf.navPluginUrls.push($(el).val()); });
$('.confAccountPluginUrl').each(function(idx, el) { moquiConf.accountPluginUrls.push($(el).val()); });
if (moqui.localeMap[moquiConf.locale]) moquiConf.locale = moqui.localeMap[moquiConf.locale];

var app = Vue.createApp({
    data: function() { return { basePath:moquiConf.basePath, linkBasePath:moquiConf.linkBasePath, currentPathList:[], extraPathList:[], currentParameters:{}, bodyParameters:null,
        ...
        moquiSessionToken:moquiConf.moquiSessionToken, appHost:moquiConf.appHost, appRootPath:moquiConf.appRootPath, userId:moquiConf.userId, username:moquiConf.username, locale:moquiConf.locale,
        reLoginShow:false, reLoginPassword:null, reLoginMfaData:null, reLoginOtp:null,
        notificationClient:null, sessionTokenBc:null, qzVue:null, leftOpen:moquiConf.leftOpen, moqui:moqui }; },
    methods: {
        setUrl: function(url, bodyParameters, onComplete, pushState=true) {
            ...
            if (this.currentLinkUrl === url && url !== this.linkBasePath) {
                ...
                fetch(menuDataUrl).then(function(r) {
                    if (!r.ok) throw new Error(r.status);
                    return r.text();
                }).then(function(outerListText) {
                    var outerList = null;
        ...
        switchDarkLight: function() {
            this.$q.dark.toggle();
            fetch(this.appRootPath + '/apps/setPreference', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: new URLSearchParams({ moquiSessionToken: this.moquiSessionToken,
                    preferenceKey: 'QUASAR_DARK', preferenceValue: (this.$q.dark.isActive ? 'true' : 'false') })
            }).catch(moqui.handleFetchError);
        },
        toggleLeftOpen: function() {
            ...
            fetch(this.appRootPath + '/apps/setPreference', {
                ...
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: new URLSearchParams({ moquiSessionToken: this.moquiSessionToken,
                    preferenceKey: 'QUASAR_LEFT_OPEN', preferenceValue: (this.leftOpen ? 'true' : 'false') })
            }).catch(moqui.handleFetchError);
        },
    ...
    created: function() {
        // All conf values already loaded into moquiConf before app.mount() cleared #apps-root.
        // data() initialized from moquiConf; only non-DOM setup remains here.
        this.$q.dark.set(moquiConf.darkMode === "true");
        ...
        this.addNavPluginsWait(moquiConf.navPluginUrls, 0);
        this.addAccountPluginsWait(moquiConf.accountPluginUrls, 0);
    },
    mounted: function() {
        // Vue 3: this.$el is the component's virtual DOM root (fragment), NOT the mount container.
        // #apps-root has display:none in the FTL; target it directly to show the app.
        ...
        // Initial Pinia store sync after root is mounted
...
// globalProperties and plugins must be set/installed before components are registered and before mount.
app.config.globalProperties.moqui = moqui;
app.config.globalProperties.moment = moment;
app.config.globalProperties.window = window;
app.config.globalProperties.$filters = {
    decodeHtml: moqui.htmlDecode,
    format: moqui.format
};

// Install plugins. Order: Pinia first, then Quasar, then http-vue-loader.
app.use(Pinia.createPinia());
// Quasar 2 UMD requires components, directives, and plugins to be passed explicitly.
// Passing the Quasar namespace object registers all. Confirmed Outcome A in Phase 0 PoC.
// Merge window.quasarConfig (set in WebrootVue.qvt.ftl) so brand/notify/loadingBar
// settings reach Quasar 2. dark:'auto' is the base; user pref applied in mounted().
app.use(Quasar, {
    config: Object.assign({ dark: 'auto' }, window.quasarConfig || {}),
    components: Quasar,
    directives: Quasar,
    plugins: Quasar
});
// http-vue-loader install() adds a beforeCreate mixin that resolves 'url:...' component strings.
...
app.component('m-dialog', {
    ...
    template:
    '<q-dialog :modelValue="modelValue" @update:modelValue="$emit(\'update:modelValue\', $event)" :id="id" @show="onShow" @hide="onHide" :maximized="$q.platform.is.mobile">' +
...
app.component('m-container-dialog', {
...
app.component('m-dynamic-container', {
...
app.component('m-dynamic-dialog', {
    name: "mDynamicDialog",
...
app.component('m-editable', {
    ...
    render: function() { return Vue.h(this.labelType, { id:this.id, 'class':'editable-label', innerHTML:this.labelValue }); }
});
...
app.component('m-form-column-config', {
...
app.component('m-date-time', {
    ...
    '<q-input dense outlined stack-label :label="label" :modelValue="modelValue" @update:modelValue="$emit(\'update:modelValue\', $event)" :rules="rules"' +
        ...
        '<template v-slot:prepend v-if="type==\'date\' || type==\'date-time\' || !type">' +
            '<q-icon name="event" class="cursor-pointer">' +
                '<q-popup-proxy ref="qDateProxy" transition-show="scale" transition-hide="scale">' +
                    '<q-date :modelValue="modelValue" @update:modelValue="$emit(\'update:modelValue\', $event); $refs.qDateProxy.hide()" :mask="formatVal"></q-date>' +
                '</q-popup-proxy>' +
            '</q-icon>' +
        '</template>' +
        '<template v-slot:append v-if="type==\'time\' || type==\'date-time\' || !type">' +
            '<q-icon name="access_time" class="cursor-pointer">' +
                '<q-popup-proxy ref="qTimeProxy" transition-show="scale" transition-hide="scale">' +
                    '<q-time :modelValue="modelValue" @update:modelValue="$emit(\'update:modelValue\', $event); $refs.qTimeProxy.hide()" :mask="formatVal" format24h></q-time>' +
                '</q-popup-proxy>' +
            '</q-icon>' +
        '</template>' +
...
app.component('m-display', {
    ...
    template:
        '<q-input v-if="labelWrapper" dense outlined readonly stack-label autogrow :modelValue="displayValue" :label="label" :id="id" :name="name" :loading="loading">' +
            '<q-tooltip v-if="tooltip">{{tooltip}}</q-tooltip>' +
        '</q-input>' +
        '<span v-else :id="id">' +
...
app.component('m-drop-down', {
    ...
    template:
        ...
        '<q-select ref="qSelect" :modelValue="modelValue" @update:modelValue="handleInput($event)"' +
...
app.component('m-text-line', {
    ...
    template:
        '<q-input :dense="dense" :outlined="outlined" :bg-color="bgColor" stack-label :label="label" :prefix="prefix"' +
                ' :modelValue="modelValue" @update:modelValue="$emit(\'update:modelValue\', $event)" :type="type"' +
...
app.component('m-ck-editor', {
    ...
    emits: ['update:modelValue', 'focus', 'blur', 'ready'],
...
app.component('m-subscreens-tabs', {
...
app.component('m-subscreens-active', {
...
app.component('m-menu-nav-item', {
    ...
    '<q-expansion-item v-if="navMenuItem && navMenuItem.subscreens && navMenuItem.subscreens.length" :model-value="true" :content-inset-level="0.3"' +
            ' switch-toggle-side dense dense-toggle expanded-icon="arrow_drop_down" :to="navMenuItem.pathWithParams" @update:model-value="go">' +
    ...
    '<q-expansion-item v-else-if="navMenuItem && navMenuItem.savedFinds && navMenuItem.savedFinds.length" :model-value="true" :content-inset-level="0.3"' +
        ...
        '<template v-slot:default><q-expansion-item v-for="(savedFind, ssIndex) in navMenuItem.savedFinds" :key="savedFind.name"' +
                ' :model-value="false" switch-toggle-side dense dense-toggle expand-icon="chevron_right" :to="savedFind.pathWithParams" @update:model-value="goPath(savedFind.pathWithParams)">' +
    ...
    '</q-expansion-item>' +
    '<q-expansion-item v-else-if="navMenuItem" :model-value="false" switch-toggle-side dense dense-toggle expand-icon="arrow_right" :to="navMenuItem.pathWithParams" @update:model-value="go">' +
        '<template v-slot:header><m-menu-item-content :menu-item="navMenuItem" active></m-menu-item-content></template>' +
    '</q-expansion-item>',
    methods: {
...
app.component('m-menu-subscreen-item', {
    ...
    '<q-expansion-item v-else :model-value="false" switch-toggle-side dense dense-toggle expand-icon="arrow_right" :to="subscreen.pathWithParams" @update:model-value="go">' +
...
app.component('m-menu-item-content', {
...
// Vue Router 4 / Quasar 2 compatible: resolve() must return a flat RouteLocation object.
// Quasar 2 destructures { matched, href, fullPath, ... } from the top-level result.
moqui.webrootRouter = {
    resolve: function resolve(to) {
        var location = moqui.isString(to) ? moqui.parseHref(to) : (to || {});
        var path = location.path || (moqui.isString(to) ? to : '/');
        if (moqui.webrootVue) path = moqui.webrootVue.getLinkPath(path);
        var lslIdx = path.lastIndexOf("/");
        var name = lslIdx === -1 ? path : path.slice(lslIdx+1);
        var href = moqui.makeHref(Object.assign({}, location, { path: path }));
        // Flat Vue Router 4 RouteLocation format -- Quasar 2 reads matched/href/fullPath at top level
        return { name:name, meta:{}, path:path, fullPath:path,
            hash:location.hash||"", query:location.query||{}, params:{},
            matched:[], href:href, redirectedFrom:undefined };
    },
    // Vue Router 4: push/replace must return a Promise (Quasar 2 calls .then() on the result)
